...
class RiskOverlay:
    ...
    def check_can_trade(self, portfolio=None) -> tuple:
        """
        取引可否をチェックします。
        ...
        if self._dd_stop:
            return False, f"DD_STOP: 最大DD {self.max_dd_pct:.1f}% 超過（現在 {portfolio.get_drawdown_rate():.1f}%）"
