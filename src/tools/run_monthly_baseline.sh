...
LOG="/tmp/baseline_monthly.log"
{
  echo "==== [$(date '+%F %T')] Baseline monthly batch start ===="
} >> "$LOG"
...
for m in "${MONTHS[@]}"; do
  ...
  echo "[Baseline] Running $TAG ($START -> $END) ..." | tee -a "$LOG"
  ...
  if [[ -f "$latest_json" ]]; then
    cp "$latest_json" "$report_dir/baseline_${TAG}.json"
    echo "Saved: $report_dir/baseline_${TAG}.json" | tee -a "$LOG"
  else
...
echo "Baseline monthly batch completed." | tee -a "$LOG"
{
  echo "==== [$(date '+%F %T')] Baseline monthly batch end ===="
} >> "$LOG"
