...
class TradingStrategy:
    ...
    def evaluate_exit(self):
        ...
        # ADDシグナルが既に設定されている場合は保存（評価順序の優先度制御）
        existing_decision = self.trade_decision.get("decision", "NONE")
        existing_side = self.trade_decision.get("side", "NONE")
        ...
        # EXITが判定されなかった場合、ADDシグナルを保持する
        if decision == 'NONE' and existing_decision == 'ADD':
            # ADDシグナルが既に設定されている場合は、それを保持
            self.trade_decision["side"] = existing_side
            self.trade_decision["decision"] = existing_decision
        else:
            # EXITが判定されたか、ADDが無い場合
            self.trade_decision["side"] = side
            self.trade_decision["decision"] = decision
            self.trade_decision["exit_reason"] = exit_reason
