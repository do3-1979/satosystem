...
class ConfigManager:
    ...
    @staticmethod
    def load_template_from_file(template_path="config.template.ini"):
        """
        config.template.ini をファイルから読み込む
        ...
        if not os.path.exists(template_path):
            raise FileNotFoundError(
                ...
                f"Please ensure config.template.ini exists in the correct location."
            )
    ...
    def create_template():
        """
        config.template.ini をファイルから読み込んで返す
        ファイルが見つからない場合のみデフォルトテンプレートを返す（後方互換性）
        """
        try:
            return ConfigManager.load_template_from_file("config.template.ini")
        except FileNotFoundError:
            # フォールバック：デフォルトテンプレートを返す
            print("⚠️  Warning: config.template.ini not found. Using default template.")
            default_template = """# Bitcoin 自動取引ボット設定ファイル
...
regime_detection_enabled = False # Phase1: SIDEWAYS regime detection
