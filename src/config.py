...
class Config:
    # ルートディレクトリを自動判定（src/の1階層上）
    _root_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    
    config = configparser.ConfigParser(inline_comment_prefixes=('#', ';'))
    config.read(os.path.join(_root_dir, 'src', 'config.ini'), encoding="utf-8_sig")
    
    # Configuration cache (initialized on first access)
    _cache = None
    
    # 一時的な config ファイルパスを指定する（backtest用）
    _config_file_path = os.path.join(_root_dir, 'src', 'config.ini')
    ...
    def get_log_dir_name(cls):
        ...
        """
        cls._initialize_cache()
        log_dir = cls._cache['log_directory']
        # ルートディレクトリ相対パスの絶対パスを返す
        return os.path.join(cls._root_dir, log_dir)
    ...
    def get_report_dir_name(cls):
        """レポートの出力ディレクトリの絶対パスを取得します。"""
        cls._initialize_cache()
        report_dir = cls._cache['report_directory']
        # ルートディレクトリ相対パスの絶対パスを返す
        return os.path.join(cls._root_dir, report_dir)
