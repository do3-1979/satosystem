...
- [x] C4: Config キャッシュ化 (2025-11-18) - 全設定値を起動時にロード、パフォーマンス向上
- [x] B5: ATR方式ボラティリティ置換 (2025-11-18) - True Range ベースの正確な計算に変更
...
### 中期
- [ ] 戦略パラメータチューニング (ATR導入により精度向上)
- [ ] 戦略分析: ADX/PSAR フィルタ統合による勝率改善
...
### C4: Config キャッシュ化
**実装内容:**
- クラスレベルキャッシュ `_cache` の導入
- `_initialize_cache()` メソッドで全設定値を1回のみロード
- 全getterメソッドをキャッシュ参照に変更（約50メソッド）
- Epoch時間、max_term などの派生値も事前計算
...
### B5: ATR 方式ボラティリティ
**実装内容:**
- `indicator_service.py` に `calculate_atr()` メソッド追加
- True Range 計算: `max(high-low, abs(high-prev_close), abs(low-prev_close))`
- `calculate_volatility()` を ATR に委譲（後方互換性維持）
- price_data_management.py、risk_management.py での自動適用

**効果:**
- ギャップを考慮した正確なボラティリティ計算
- ポジションサイズ・STOP幅の精度向上
- 市場急変時の安定性改善

**検証:**
- 2日間バックテスト (2025/11/09-11/11) で動作確認
- ATR値: 598-1280 範囲で適切に計算
- PnL: 72.39 USD, Trades: 1, 正常動作確認
