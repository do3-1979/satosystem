...
## ⚠️ 重大ルール：コミット・プッシュの許可取得

**すべてのコミットとプッシュは、ユーザーの明示的な許可を得た後にのみ実行する。**

### 理由
- **セキュリティ**: APIキーが config.ini に含まれている場合、意図しないプッシュで認証情報が公開される危険性
- **意図確認**: 修正内容がユーザー指定の要件に合致しているか、事前にレビューを受ける必要
- **エラー回避**: テスト未実施のコードが公開されることを防止
...
### 禁止事項
- ❌ ユーザーの許可なくコミットすること
- ❌ 複数の変更を一度にコミット・プッシュすること（ファイルごと、機能ごとに分割）
- ❌ テスト未実施の状態でプッシュすること
- ❌ APIキーが config.ini に含まれた状態でプッシュすること（bot_run.sh で置換されるまで待機）
...
- 最新の品質指標（2025-12-29）：
  - **レグレッション テスト**: 54テスト中90成功（成功率97.8%） ✅ 
  - **四半期別テスト**: 2024Q2～2025Q4 7四半期実施、累積損益 +374.95 USD ✅ (Q1除外)
  - **データ型検証**: ExitStrategyV2 修正により bgmode エラー完全解決 ✅
  - **パラメータ最適化**: 最終パラメータセット確定 (vol=28, don=32, entry=3, stop=2) ✅
