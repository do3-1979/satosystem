...
実行方法:
  ...
  python3 test_live_order_xaut.py

注意:
  - config_xaut.ini の back_test=1 を 0 に、hot_test_dummy_mode=1 を 0 に
    一時的に書き換えてテストを実施します（テスト後に自動復元）
  - 最小注文量 0.01 XAUT（≒ $33 USD）を使用します
"""
...
# テストパラメータ（変更可能）
# =====================================================
TEST_QUANTITY = 0.01       # テスト注文数量（XAUT）※ Bitget最小 0.01 XAUT 推奨
...
def patch_config_for_live():
    """config_xaut.ini を本番モード（back_test=0, hot_test_dummy_mode=0）に書き換え"""
    ...
    with open(CONFIG_PATH, 'w') as f:
        cfg.write(f)
    print("✅ config_xaut.ini → 本番モード（back_test=0, hot_test_dummy_mode=0）に変更")


def restore_config():
    """config_xaut.ini をテスト前の状態に復元"""
    if os.path.exists(CONFIG_BACKUP_PATH):
        shutil.copy2(CONFIG_BACKUP_PATH, CONFIG_PATH)
        os.remove(CONFIG_BACKUP_PATH)
        print("✅ config_xaut.ini を元に戻しました")
    else:
        print("⚠️  バックアップが見つかりません。config_xaut.ini を手動確認してください")


def get_exchange():
    """BitgetExchange インスタンスを XAUT 設定で生成"""
...
def check_position(exchange):
    ...
    try:
        ...
        if open_pos:
            for p in open_pos:
                print(f"  📌 ポジション: {p['side']} {p.get('contracts', '?')} XAUT @ {p.get('entryPrice', '?')} USD  未実現損益: {p.get('unrealizedPnl', '?')} USDT")
        else:
            print("  📌 オープンポジション: なし")
        return open_pos
...
def test_market_entry_exit(exchange):
    ...
    print(f"  注文数量: {TEST_QUANTITY} XAUT（≒ {TEST_QUANTITY * current_price:,.2f} USD）")
...
def main():
    print_sep("Bitget 本番注文テスト（XAUT/USDT）")
    print("  ⚠️  このテストは実際の資金を使用します")
    print("  ⚠️  Bitgetダッシュボードを開いて監視しながら実行してください")
    print(f"\n  テストパラメータ:")
    print(f"    シンボル    : XAUT/USDT（金現物トークン先物）")
    print(f"    注文数量    : {TEST_QUANTITY} XAUT")
